...
export default function TradesPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const tokens = JSON.parse(localStorage.getItem("tokens"));
      // Set trade_date to selected day if adding from day view
      const dataToSubmit = {
        ...formData,
  ...
  const generateCalendar = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      days.push(<div key={`empty-${i}`} className="h-24 sm:h-32 bg-[#0a0a0a]"></div>);
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          ...
          className={`h-24 sm:h-32 bg-[#141414] border border-neutral-800 rounded-lg p-2 sm:p-3 cursor-pointer hover:border-neutral-600 transition-all ${isToday ? 'ring-2 ring-blue-500' : ''}`}
        >
          <div className="flex flex-col h-full">
            <div className="text-sm sm:text-base font-semibold text-white mb-1">{day}</div>
            {dayTrades.length > 0 && (
              <>
                <div className="text-xs sm:text-sm text-neutral-400 mb-1">
                  {dayTrades.length} trade{dayTrades.length !== 1 ? 's' : ''}
                </div>
                <div className={`text-sm sm:text-lg font-bold mt-auto ${pnl >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                  ${pnl.toFixed(2)}
                </div>
              </>
            )}
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-neutral-400">Click on any day to view or add trades</p>
          </div>
          <button
            onClick={() => {
              setSelectedDay(null);
        ...
        {/* Month Navigation */}
        <div className="flex items-center justify-between mb-6 bg-[#141414] border border-neutral-800 rounded-xl p-4">
          <button
            onClick={prevMonth}
        ...
        {/* Calendar Grid */}
        <div className="bg-[#141414] border border-neutral-800 rounded-xl p-4 sm:p-6">
          {/* Day names */}
      ...
      {showDayModal && selectedDay && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={() => setShowDayModal(false)}>
          <div className="bg-[#141414] border border-neutral-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-[#141414] border-b border-neutral-800 p-6 flex items-center justify-between">
              <h3 className="text-2xl font-bold text-white">
                Trades for {new Date(selectedDay).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
              </h3>
            ...
            <div className="p-6">
              <button
                ...
                className="w-full mb-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-medium rounded-lg transition-all"
              >
              ...
              {getTradesForDay(parseInt(selectedDay.split('-')[2])).length === 0 ? (
                ...
                <div className="space-y-4">
                  {getTradesForDay(parseInt(selectedDay.split('-')[2])).map((trade) => (
                    <div key={trade.id} className="bg-[#0a0a0a] border border-neutral-800 rounded-lg p-4 hover:border-neutral-700 transition-colors">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <h4 className="text-lg font-bold text-white">{trade.symbol}</h4>
                          <p className="text-sm text-neutral-400">{new Date(trade.trade_date).toLocaleTimeString()}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className={`text-xl font-bold ${parseFloat(trade.pnl) >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                      ...
                      {trade.notes && (
                        <p className="mt-3 text-sm text-neutral-400 italic">{trade.notes}</p>
                      )}
                    </div>
                  ))}
                </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={() => setShowAddModal(false)}>
          <div className="bg-[#141414] border border-neutral-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full px-4 py-3 bg-[#0a0a0a] border border-neutral-700 rounded-lg text-white placeholder-neutral-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                    placeholder="EURUSD"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className="w-full px-4 py-3 bg-[#0a0a0a] border border-neutral-700 rounded-lg text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  >
              ...
              <button
                type="submit"
                className="mt-6 w-full py-3 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-medium rounded-lg shadow-lg shadow-blue-500/20 transition-all"
              >
                Add Trade
              </button>
            </form>
